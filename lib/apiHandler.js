    ...
    if (pathname.startsWith('/api/v1/') && lastPart === 'customers' || lastLastPart === 'customers') {
        if (lastPart === 'customers') {
            if (method === 'GET') { // GET (ALL)
                ...
                request.on('end', () => {
                    let newCustomer = JSON.parse(body);
                    newCustomer.id=999;

                    customers.push(newCustomer);
                    response.writeHead(201, { 'Content-Type': 'application/json' });
                    response.end(JSON.stringify(newCustomer));
                    // response.end(JSON.stringify({ 'POST': 'server error, methos not implementrd' }));
                })

                // response.writeHead(500, { 'Content-Type': 'application/json' });
                // response.end(JSON.stringify({ 'message': 'server error, methos not implementrd' }));
            }
        } else {
